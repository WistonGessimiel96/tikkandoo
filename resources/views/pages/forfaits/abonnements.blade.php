...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Liste des forfaits type abonnement du TR</h3> <br>
            <button type="button" class="btn btn-info" data-toggle="modal" data-target="#modal-add-type">
                Ajouter un forfait abonnement
            </button>
                ...
                <div class="modal-dialog modal-xl">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="post" action="{{url('/forfaits/abonnements')}}">
                                @csrf()
                                <div class="row">
                                    <div class="col-4">
        ...
        <div class="card-body">
            <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach($users as $user)
                    <tr>
                        <td>{{$user->nom_forfait}}</td>
                        <td>{{$user->zone_forfait}}</td>
                        <td>{{$user->prix_forfait}} Fcfa</td>
                        <td>
                            <button type="button" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#modal-add-type-{{$user->id}}">
                                Modifier le forfait abonnement
                            </button>
                            <div class="modal fade" id="modal-add-type-{{$user->id}}" style="display: none;" aria-modal="true" role="dialog">
                                <div class="modal-dialog modal-xl">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form method="post" action="{{url('/forfaits/abonnements/'.$user->id)}}">
                                                @csrf()
                                                <input type="hidden" value="{{$user->id}}" name="id">
                                                <div class="row">
                                                    <div class="col-4">
                                                        <input type="text" name="nom_ticket" value="{{$user->nom_forfait}}" class="form-control" placeholder="nom du forfait ticket" required>
                                                        <br>
                                                    </div>
                                                    <div class="col-4">
                                                        <select name="destination" id="" class="form-control" required>
                                                            <option value="">choisir la zone</option>
                                                            <option value="zone_1" {{"zone_1" == $user->zone_forfait ? "selected" : ""}}>1 Zone</option>
                                                            <option value="zone_2" {{"zone_2" == $user->zone_forfait ? "selected" : ""}}>2 Zones</option>
                                                            <option value="zone_3" {{"zone_3" == $user->zone_forfait ? "selected" : ""}}>3 Zones</option>
                                                        </select> <br>
                                                    </div>
                                                    <div class="col-2">
                                                        <input type="text" name="prix" value="{{$user->prix_forfait}}" class="form-control" placeholder="prix" required>
                                                        <br>
                                                    </div>
                                                    <div class="col-4">
                                                        <button type="submit" class="btn btn-warning">Enregistrer</button>
